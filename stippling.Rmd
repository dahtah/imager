...
voronoi(xy,res,res) %>% plot
points(xy,col="red",pch=19)
```

To get the center of mass of each cell, we use dplyr: 

```{r}
library(dplyr)
xy.c <- voronoi(xy,res,res) %>% as.data.frame %>%
...
plot(out,ylim=c(height(im),1),cex=.2,pch=19,axes=FALSE,xlab="",ylab="")
```
...
Another variant in colour: 

```{r}
stipple.colour <- function(im,nDots=1e4,scale=40)
{
    out <- grayscale(im) %>% stipple(nDots)
    col <- interp(im,out) %$% rgb(c.1,c.2,c.3)
    g <- grayscale(im) %>% imgradient("xy") %>% map(~ isoblur(.,3)) %>% map(~ interp(.,out))
    plot(out,ylim=c(height(im),1),cex=.5,pch=19,col="white",axes=FALSE,xlab="",ylab="")
    segments(out$x,out$y,out$x-scale*g$y,out$y+scale*g$x,col=col,lwd=3)
}


load.example("parrots") %>% isoblur(3) %>% stipple.colour 
```
