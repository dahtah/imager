...
If you don't want imager to rescale the colours automatically, set rescale.color to FALSE, but now imager will want values that are in the $[0,1]$ range. 

```{r}
layout(t(1:2))
boats <- boats/255 #scale to 0..1
plot(boats,rescale.col=FALSE)
plot(boats/2,rescale.col=FALSE)
```
...
plot(boats/255,colourscale=cscale,rescale=FALSE)
```
...
Have a look around the [website](http://dahtah.github.io/imager). 
