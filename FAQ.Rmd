...
# How do I convert images in magick format? 

*magick* is another R package for image processing, interfacing the ImageMagick library. magick uses its own internal format for images, so there's no way to directly convert images for use with imager, but you can go through R's raster format: 

```{r}
library(magick)
frink <- image_read("https://jeroenooms.github.io/images/frink.png")
as.raster(frink) %>% as.cimg %>% plot
```
